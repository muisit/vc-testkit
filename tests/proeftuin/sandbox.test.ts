...
describe('Sandbox Issuer', () => {
    ...
    if (!suiteContains('proeftuin') || testDisabled('ISSUER_SANDBOX_TOKEN')) {
        it.skip("disabled");
        return;
    }
    ...
    const cases = [
      ...
      { name: tenant + ' PID as JWT', input: { credential: PID_JWT } },
      { name: tenant + ' GenericCredential as JWT', input: { credential: GC_JWT } },
      { name: tenant + ' GenericCredential as JWTv1', input: { credential: GC_JWT_1 } },
    ];
